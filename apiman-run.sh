...
if docker ps -a -f name=$NAME | grep -q $NAME; then
	...
	#We don't expose the HTTP port 8080 directly. It is proxied through Nginx
	docker run --name $NAME -p 8080:8080 -d -e VIRTUAL_HOST=ooapi.ai.finalist.nl,ooapi.ai.finalist.lcl $DOCKER_IMAGE:$DOCKER_TAG 
fi
